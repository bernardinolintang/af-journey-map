// Static Anytime Fitness Singapore location data
// These are embedded in the frontend so the map works without any DB setup.
// Sources: anytimefitness.sg/locations (Central SG, North East, North West, South East, South West)
...
// IDs are deterministic slugs so they can be matched against DB visits
function makeId(name: string): string {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
}

const raw: Omit<StaticLocation, 'id'>[] = [
  // ── CENTRAL ──────────────────────────────────────────────────────────────
  ...
  { name: 'Bukit Timah - Beauty World',  address: '144 Upper Bukit Timah Road #B2-01, Singapore 588177',      lat: 1.3421, lng: 103.7762, country: 'SG', region: 'Central',    is_24h: true },
  ...
  { name: 'Loyang - Pasir Ris East',     address: '258 Pasir Ris Street 21 #02-01, Singapore 510258',       lat: 1.3700, lng: 103.9750, country: 'SG', region: 'South East',  is_24h: true },
...
export const STATIC_LOCATIONS: StaticLocation[] = raw.map(loc => ({
  ...loc,
  id: makeId(loc.name),
}));
