...
export async function generateShareCard(
  ...
  if (options.displayName) {
    ...
    ctx.fillText(`${options.displayName}'s AF Tracker`, W / 2, 108);
  }

  ctx.fillStyle = "rgba(255,255,255,0.55)";
  ctx.font = '500 15px "DM Sans", system-ui';
  ctx.textAlign = "center";
  ctx.fillText("I've visited", W / 2, options.displayName ? 128 : 118);

  // ── Big number ───────────────────────────────────────────────────────────
  ctx.fillStyle = "#ffffff";
  ctx.font = `800 86px "Bebas Neue", "Oswald", "Barlow Condensed", system-ui`;
  ctx.textAlign = "center";
  ctx.fillText(`${visited}`, W / 2, options.displayName ? 216 : 206);

  // ── "/ total outlets" ────────────────────────────────────────────────────
  ctx.fillStyle = "rgba(255,255,255,0.45)";
  ctx.font = '500 16px "DM Sans", system-ui';
  ctx.fillText(`/ ${total} Anytime Fitness outlets`, W / 2, options.displayName ? 242 : 232);

  // ── "in Singapore" ───────────────────────────────────────────────────────
  ctx.fillStyle = "rgba(255,255,255,0.35)";
  ctx.font = '400 13px "DM Sans", system-ui';
  ctx.fillText("in Singapore", W / 2, options.displayName ? 264 : 254);

  // ── Progress bar ─────────────────────────────────────────────────────────
  const barX = 40,
    barY = options.displayName ? 282 : 272,
    barW = W - 80,
    barH = 10,
    barR = 5;
  // Track
  ...
  ctx.textAlign = "center";
  ctx.fillText(`${percentage}%`, W / 2, options.displayName ? 320 : 310);

  // ── Highlights ───────────────────────────────────────────────────────────
  if (options.topRegion || options.nextTarget) {
    ctx.fillStyle = "rgba(255,255,255,0.45)";
    ctx.font = '500 11px "DM Sans", system-ui';
    ctx.textAlign = "center";
    const highlights = [
      options.topRegion ? `Top region: ${options.topRegion}` : "",
      options.nextTarget ? `Next target: ${options.nextTarget}` : "",
    ]
      .filter(Boolean)
      .join("  ·  ");
    ctx.fillText(highlights, W / 2, options.displayName ? 338 : 328);
  }
